...
pub struct InteractiveConsoleClient<'a> {
    process_name: Option<&'static str>,
    uart_client: &'a uart::Client<'a>,
    tx_buffer: &[u8],
    rx_buffer: &[u8],
}

pub struct InteractiveConsoleMux<'a> {
    uart: &'a uart::UartData<'a>,
    clients: [TakeCell<'static, InteractiveConsoleClient>; 4],
    active_client_index: OptionalCell<u8>,

    tx_in_progress: Cell<bool>,
    tx_buffer: TakeCell<'static, [u8]>,
    rx_in_progress: Cell<bool>,
    rx_buffer: TakeCell<'static, [u8]>,
    client_select_buffer: TakeCell<'static, [u8]>,
    client_select_index: Cell<usize>,
    running: Cell<bool>,
}

impl<'a> InteractiveConsoleMux<'a> {
    pub fn new(
        uart: &'a uart::UartData<'a>,
        ...
        ProcessConsole {
            uart: uart,
            clients: [OptionalCell::empty(), 4],
            active_client_index: OptionalCell::empty(),
            ...
            client_select_buffer: TakeCell::new(cmd_buffer),
            client_select_index: Cell::new(0),

            running: Cell::new(false),
        }
    }

    pub fn register(&self, client: &'a uart::Client<'a>) -> ReturnCode {
...
impl<'a> uart::TransmitClient for InteractiveConsoleMux<'a> {
...
impl<'a> uart::ReceiveClient for InteractiveConsoleMux<'a> {
    fn received_buffer(
        &self,
