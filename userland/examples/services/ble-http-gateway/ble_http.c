...
#define BLEHTTP_BASE_UUID {0x30, 0xb3, 0xf6, 0x90, 0x9a, 0x4f, 0x89, 0xb8, \
                           0x1e, 0x46, 0x44, 0xcf, 0x01, 0x00, 0xba, 0x16}
#define BLE_UUID_BLEHTTP_SERVICE     0x0001
#define BLE_UUID_BLEHTTP_HTTP_CHAR   0x0002
  ...
  switch (p_ble_evt->header.evt_id) {
    ...
    case BLE_GATTC_EVT_CHAR_DISC_RSP: {
      if (p_ble_evt->evt.gattc_evt.gatt_status != BLE_GATT_STATUS_SUCCESS) {
        ...
        ble_uuid_t httpget_http_characteristic_uuid = {
          .uuid = BLE_UUID_BLEHTTP_HTTP_CHAR,
          .type = BLE_UUID_TYPE_VENDOR_BEGIN,
        };
        ...
        for (int i = 0; i < p_char_disc_rsp->count; i++) {
          ...
          } else if (BLE_UUID_EQ(&httpget_http_characteristic_uuid, &(p_char_disc_rsp->chars[i].uuid))) {
            _char_handle_http = p_char_disc_rsp->chars[i].handle_value;
            // printf("found https char handle 0x%x\n", _char_handle_http);
        ...
        if (_char_handle_https == 0 || _char_handle_http == 0 || _char_handle_body == 0) {
